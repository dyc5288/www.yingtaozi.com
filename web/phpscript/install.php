...
if (!empty($flag['ytz_posts']))
{
    ...
    $sql    = "CREATE TABLE IF NOT EXISTS `%s` (
            ...
            `post_date` datetime NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '添加时间',
            `post_date_gmt` datetime NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '格林标准添加时间',
            `post_content` longtext NOT NULL COMMENT '文章内容',
            `post_title` text NOT NULL COMMENT '文章内容',
            `post_excerpt` text NOT NULL,
            `post_status` varchar(20) NOT NULL DEFAULT 'publish' COMMENT '文章状态：inherit：继承；auto-draft：自动草稿；publish：公开；pending： 等待复审',
